...
% Stand alone testing script for isetbio (to compute a single simple experiment)
% by EK (May 29, 2018)
...
% Show stimulus:
figure; 
for frame = 1:size(ois.oiModulated.data.photons,3) 
    imagesc(ois.oiModulated.data.photons(:,:,frame)); 
    colormap gray; 
    title(sprintf('OIS frame %d', frame)); 
    drawnow; pause(0.1);
end
