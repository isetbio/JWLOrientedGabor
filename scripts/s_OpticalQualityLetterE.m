...
whichGroup  = 'singleRandomEmmetrope'; %group(2).RE(randi(length(group(2).RE),1));% 'emmetropes'; % can also be myopes
...
for ecIdx = 1:length(eccen)
    opticalQuality = conv2(E.im,PSF_centered(:,:,ecIdx),'same');
    ...
    subplot(241);
%     support_Letter_ArcMin = size(centeredLetterDownsampled,1)*samples2Arcmin;
    ...
    set(gca, 'TickDir', 'out'); box off; 
    axis equal
    axis([-1 1 -1 1]*max(support_samples)/2); 
%     idx = idx+1;
