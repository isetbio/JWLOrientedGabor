...
% To simulate figure 5 from Jaeken & Artal at 5 different locations, we match
% the letter E to have a size 30 arc min
...
% Locations of the wavefronts (x,y) in deg
locationsRetina = {[0,-5], [5,0], [0,0], [-5,0], [0,5]};
locationsVisualField = {[0,5], [-5,0], [0,0], [5,0], [0,-5]};

% Plotting locations and labels
subplotLoc = [3,6,7,8,11];
subplotRetinalCoords = {'Superior retina', 'Nasal retina', 'Fovea', 'Temporal retina', 'Inferior retina'};
subplotVisualCoords = {'Lower visual field', 'Left visual field', 'Fovea', 'Right visual field', 'Upper visual field'};

% What specific subject? (choose from 1-10, except 2)
subject  = 10;

%% 2. Loop over eccentricities to define wavefront

for ec = 1:numel(locationsRetina)
    
    clear wvf oi;
    
    % Get wavefront aberrations from Jaeken and Artal 2012 dataset
    [wvf] = wvfLoadWavefrontOpticsData('wvfZcoefsSource', 'Polans2015', 'jIndex', 3:14, 'whichEye', 'right', 'eccentricity', locationsRetina{ec}, 'whichGroup', subject, 'relativeRefraction', true, 'verbose', true);
...
for ecIdx = 1:numel(locationsVisualField)
    ...
    colormap gray;
    xlabel('arc min'); ylabel('arc min'); title(sprintf('%s (X:%d, Y:%d)',subplotVisualCoords{ecIdx}, locationsVisualField{ecIdx}(1),locationsVisualField{ecIdx}(2)))
